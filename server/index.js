...
const rooms = new Map();
const now = () => Date.now();
function room(id) { if (!rooms.has(id)) rooms.set(id, { video: null, playback: { state: 'paused', currentTime: 0, updatedAt: now() }, clients: new Map() }); return rooms.get(id); }
...
function participants(r) { return [...r.clients.values()].map(({ id, name }) => ({ id, name })); }
...
wss.on('connection', (ws, request) => {
  ...
  const r = room(roomId); const user = { id: randomUUID(), name: `Guest ${r.clients.size + 1}` };
  r.clients.set(ws, user);
  ws.send(JSON.stringify({ type: 'welcome', you: user, video: r.video, playback: { ...r.playback, currentTime: actualTime(r), updatedAt: now() }, participants: participants(r), serverTime: now() }));
  sendRoster(r);
  ws.on('message', raw => {
    ...
    if (!r.video || !['play', 'pause', 'seek', 'ended', 'syncRequest'].includes(msg.type)) return;
    ...
    emit(r, { type: msg.type, time, timestamp: r.playback.updatedAt, by: user.id });
  });
